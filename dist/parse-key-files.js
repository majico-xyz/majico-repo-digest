...
export function selectKeyFilesFromTree(tree) {
    const hits = [];
    for (const entry of tree) {
        if (entry.type !== "blob")
            continue;
        if (classifyRepoPath(entry.path))
    ...
    for (const path of paths) {
        ...
        if (kind === "package_json") {
            try {
                ...
                if (pkg.name)
                    parsed.productName = pkg.name;
                if (pkg.description)
                    parsed.oneLiner = pkg.description;
            }
            catch {
                /* ignore invalid json */
            }
        ...
        if (kind === "readme" && !parsed.oneLiner) {
            ...
            if (firstLine)
                parsed.oneLiner = firstLine.trim().slice(0, 200);
        }
        if (kind === "brand_md") {
            const storyMatch = content.match(/##\s*Brand story\s*\n+([\s\S]*?)(?=\n##|\n$)/i);
            if (storyMatch?.[1])
